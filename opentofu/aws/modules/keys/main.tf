...
resource "null_resource" "upload_global_jwt_values_yaml" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws s3 cp ${local.global_values_jwt_file_location} s3://${var.storage_bucket_private}/${var.environment}-global-values-jwt-tokens.yaml
    EOT
  }
...
resource "null_resource" "upload_global_rsa_values_yaml" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws s3 cp ${local.global_values_rsa_file_location} s3://${var.storage_bucket_private}/${var.environment}-global-values-rsa-keys.yaml
    EOT
  }
